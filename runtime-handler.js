...
async function startDiscordAuth(request, env) {
  console.log('[discord-start] route hit', { path: new URL(request.url).pathname, method: request.method });
  ...
  const hasClientSecret = Boolean(env.DISCORD_CLIENT_SECRET);
  const hasBaseUrl = Boolean(env.BASE_URL || env.SITE_URL);

  console.log('[discord-start] env presence check', {
  ...
  console.log('[discord-start] authorize URL build success', { hasAuthorizeUrl: Boolean(authUrl.toString()) });
  const response = redirect(authUrl.toString(), {
    'Set-Cookie': `ims_discord_state=${encodeURIComponent(state)}; ${cookieAttrs(600)}`,
  });
  console.log('[discord-start] redirect response creation', { status: response.status });
  return response;
}
